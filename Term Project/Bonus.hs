    ...
    where
        ...
        findClosest index list   = if index == (length colors) then result 
            ...
            where
                ...
                calculateError = sum $Prelude.map (\x -> x * x) [(r - (dif!!0)), (g - (dif!!1)), (b - (dif!!2))]
                result = findElement list (minimum list)  --Returns the position of the color that caused least error.



--Convert a trio to list
convertToList::(Integer,Integer,Integer) -> [Integer]
convertToList (x,y,z) = [x,y,z]


--Gets position of the given element from a list
findElement::[Integer]->Integer->Int
findElement list n = find list n 0
    where
